...
# East rooftop (2.5m, ramp-only) — 在东街东侧（贴东边界墙），不占东街走廊。
#   东街走廊 x∈[9.5,13.5]（4m，与大厅东口直通）；屋顶 x∈[15.5,21.5]；斜坡 x∈[21.5,24.5]（从东街末端上坡）。
const EAST_ROOF := {"name": "RoofEast", "kind": "roof", "center": Vector3(18.5, 1.25, 0), "size": Vector3(6, 2.5, 12)}
const EAST_RAMP := {"name": "RampEast", "kind": "ramp", "center": Vector3(23, 1.25, 0), "size": Vector3(3, 2.5, 12)}
...
const COVERS := [
	...
	# east containers (2.2m high cover) — 东街内，不挡走廊（x∈[10.5,15.5]）
	{"name": "ContainerE1", "kind": "cover", "center": Vector3(13, 1.1, -7), "size": Vector3(5, 2.2, 2.5)},
	{"name": "ContainerE2", "kind": "cover", "center": Vector3(13, 1.1, 4), "size": Vector3(5, 2.2, 2.5)},
