...
# 每点输出 OK / OK(瞬移) / FAIL + 汇总。门禁（2026-08-12 F3 返工强化）：
# 瞬移回退计入失败——仅允许起点放置 1 次瞬移（ALLOWED_TELEPORTS），
# 失败 0 且瞬移 ≤ 限额才 EXIT=0，否则 EXIT=1（连通性硬门控，防封路回归）。
#
...
const ALLOWED_TELEPORTS := 1  # 2026-08-12 F3 返工：瞬移仅许起点放置；超限 = 连通性门禁失败
...
const ROUTE := [
	...
	["引·横脊墙北绕", Vector3(-19, 0.25, 8.0)],  # 2026-08-12 F3 横脊墙绕行：下塔经塔坡道北行，走 WestSpurN 北侧（z>7.5）
	["引·下塔西", Vector3(-15.3, 0, 8.5)],
	# ---- rim 西口 + 南口 ----
	["引·横脊墙北侧绕", Vector3(-20, 0.5, 7.1)],  # 2026-08-12 F3 横脊墙绕行：经塔坡道台阶顶绕 WestSpurN 西端
	["引·横脊墙北侧回", Vector3(-15.3, 0, 6.0)],  # 2026-08-12 F3 横脊墙绕行：回塔-rim 走廊（z<6.7 净空区）
	["引·rim外西南", Vector3(-15.3, 0, 0.5)],
	["rim西口", Vector3(-13.5, 0, 0)],
	["引·横脊墙南绕走廊", Vector3(-15.3, 0, -6.0)],  # 2026-08-12 F3 横脊墙绕行：先走塔-rim 走廊南下（避 WestClusterS_Panel）
	["引·横脊墙南侧绕", Vector3(-20, 0, -7.1)],  # 2026-08-12 F3 横脊墙绕行：绕 WestSpurS 西端（开放地面净宽 5m）
	...
	["引·直道横脊墙南绕", Vector3(-20, 0, -7.1)],  # 2026-08-12 F3 横脊墙绕行：绕 WestSpurS 西端
	["引·直道中段", Vector3(-15.3, 0, 0)],  # 2026-08-12 F3 横脊墙绕行：回塔-rim 走廊（避西塔坡道/塔体）
	["引·直道横脊墙北绕", Vector3(-20, 0.5, 7.1)],  # 2026-08-12 F3 横脊墙绕行：经塔坡道台阶顶绕 WestSpurN 西端
...
func _run() -> void:
	...
	quit(0 if _fail == 0 and _teleported <= ALLOWED_TELEPORTS else 1)
