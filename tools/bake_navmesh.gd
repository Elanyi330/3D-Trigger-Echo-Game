...
const RAMP_SLOPES := [
	{"x0": -21.0, "x1": -18.5, "z0": 8.2, "z1": 2.0, "y0": 0.0, "y1": 2.5, "steps": 10, "side_plates": true},
	{"x0": 18.5, "x1": 21.0, "z0": -8.2, "z1": -2.0, "y0": 0.0, "y1": 2.5, "steps": 10, "side_plates": true},
...
func _init() -> void:
	...
	# 坡道侧壁板（四坡道，2026-08-15 T-nav-ext 实测 + 2026-08-16 塔坡道重试实验）：
	# 台阶实体的侧立面封板——薄片斜坡在坡道 x 侧缘与相邻表面（塔侧地面 0 / 祭坛平台
	# 0.6）的高差 ≤0.62 时会被寻路当入口穿越（phantom side entry：祭坛坡道实测在
	# x=6.6 东缘穿入 → 楔入台阶盒东壁 stuck；实机 r8 东塔区 26 stuck = 同款镜像——
	# 角色 (18.00,0.92,-6.99) 全速压 EastTowerRamp 西立面，走廊路径从坡道西侧中段切入
	# 压 0.75m 台阶立面 > step-up 0.62）。物理上台阶盒侧立面整面封死、只能从坡脚入——
	# 用垂直无顶四边形封住 x0/x1 两侧（z0..z1、y0..y1；无顶面→不产生步行面，实测薄盒
	# 顶面会被烘成步行面并桥接成新倾斜多边形）。坡脚（z=z0）与坡顶（z=z1）出入口
	# 保持开放。塔坡道 2026-08-15 曾加板回退 WestTowerBox（寻路径形被改）；walker 侧
	# 已进化（踢面正压转向/前瞻转向/F-degen），控制器裁决重试——实验裁定：冒烟/塔区
	# 全绿则保留，任一回退则回滚。
