...
# 开镜/关镜（CS 式机瞄）：active 时 FOV ÷multiplier + 灵敏度 ÷multiplier；非 active 恢复原值
func set_ads(active: bool, multiplier: float) -> void:
	...
	ads_active = active
	if ads_active:
		cam.fov = _base_fov / ads_multiplier
		mouse_sensitivity = _base_sensitivity / 1000.0 / ads_multiplier
	else:
		if _base_fov > 0.0:
			cam.fov = _base_fov  # 防御：未开镜过就关镜（不应发生）不改 FOV
		mouse_sensitivity = _base_sensitivity / 1000.0
