...
#         摊阁↔门柱 z 缝 2.7（F7 2026-08-12：柱深 2.25→1.5 后）----
func test_belt_pavilions() -> void:
	for tags0 in [["BeltN", "GateN"], ["BeltS", "GateS"]]:
		...
		for lr in ["W", "E"]:
			...
			assert_almost_eq(gap_pillar, 2.7, 0.01,
				"%s_Pav%s 与门柱 z 缝 == 2.7（F7 柱深 2.25→1.5，2026-08-12）" % [belt, lr])


# ---- 31b. 摊阁台阶 ↔ 门柱净距 ≥1.95（F7 用户二次反馈：1.2m 通道仍卡，
#          柱深 2.25→1.5 靠广场侧再收缩 0.75m → 台阶与门柱净距恰 1.95，数据断言钉死 A）----
func test_pavilion_step_clearance() -> void:
	for tags0 in [["BeltN", "GateN"], ["BeltS", "GateS"]]:
		...
		for lr in ["W", "E"]:
			...
			# FP 容差：14.5−12.55 浮点得 1.9499999（1 ULP）——容差 0.001 仍远小于
			# F7 修复目标 1.2→1.95 的差距（若柱深回归 2.25，gap 会跌回 1.2，必失败）
			assert_gte(gap, 1.95 - 0.001, "%s_PavStep%s 与 %s_Pillar%s 水平净距 ≥ 1.95（F7 修复）"
					% [belt, lr, gate, lr])
			var pz: Vector3 = pillar["size"]
			assert_almost_eq(pz.z, 1.5, 0.001,
					"%s_Pillar%s size.z == 1.5（F7 柱深 2.25→1.5）" % [gate, lr])
