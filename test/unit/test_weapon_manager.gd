...
func before_each() -> void:
	...
	# 改变弹孔随动 y 断言——before_each 加地面盒让敌人落定 y=0（= 原静态桩站位），断言一字未改。
	_make_floor()
...
func test_switch_blocked_during_reload_completes_normally() -> void:
	...
	var reload_finished_count: Array[int] = []  # lambda 按值捕获局部原始值：用数组承载计数（同 test_hit_enemy 约定）
	ak_core.reload_finished.connect(func() -> void: reload_finished_count.append(1))
	Input.action_press("fire")
	...
	assert_eq(reload_finished_count.size(), 1, "换弹正常完成发 reload_finished")
