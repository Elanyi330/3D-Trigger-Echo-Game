...
**报告日期：** 2026-08-12
**项目状态：** M0 + M1 + M1.5 + M1.75 全部完成；**M2 进行中——小图 v3「回声祭坛」+ 手感修复轮（step-up/空中控制/横脊墙）完成**（GUT 272/272），用户待实机验收；TDM 框架/视觉/navmesh 待做
...
- ✅ **基线回写（2026-08-12 全绿）**：GUT 272/272（25 脚本，31852 断言）；193 实体；窄缝扫描 0 处；漫游探针 79 点（行走 78/瞬移 1=起点）；视线硬门控 0 对 + 外环基线 178 对（BASELINE_2026-08-12）；首交火 t=4.93s
...
6. ✅ **M2 手感修复轮完成（2026-08-12）**：台阶 step-up 0.62m（F1+F2a×2）+ 空中控制（F2b：静止跳 REST 3.0 转向/跑跳 RUN 0.76 不加速/落地钳制 6.35）+ 横脊墙×4（F3，缺口门墙封豁事故后返工删除）+ MOVEMENT_REV 记录失效键；GUT 272/272，门禁全过（0 窄缝/79 点漫游/t=4.93s/硬门控 0 对+外环基线 178 对）。用户待实机验收。
