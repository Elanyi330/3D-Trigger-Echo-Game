...
**报告日期：** 2026-08-11
**项目状态：** M0 + M1 + M1.5 + M1.75 全部完成；**M2 进行中——小图 v3「回声祭坛」（布局重写 + 波次刷新 + 跳跃记录）完成**（GUT 245/245），用户待实机验收；TDM 框架/视觉/navmesh 待做
...
| M2 地图与模式 | 小图（团战）+ 大图（占点）基础布局 + 两种模式框架 | 双模式核心循环可玩 | 🟡 进行中（小图 v3「回声祭坛」布局+波次刷新+跳跃记录完成 2026-08-11；TDM 框架/视觉/navmesh 待做） |
...
5. ✅ **M2 v3「回声祭坛」完成（2026-08-11）**：布局重写（189 实体，钟楼枢纽/祭坛台双坡道/市集带/背街/营/外环角场 + 14 刷怪面）+ 波次刷新（5 敌/波，全灭 1.5s 下一波）+ 跳跃记录（布局哈希重置铁律）；GUT 245/245，探针门禁全过（0 窄缝/71 点漫游/t=4.93s/硬门控 0 对+外环基线 195 对）。用户待实机验收。
6. **M2 剩余**：视觉升级（map_visuals 已指 v3，Kenney 方案待重做）、TDM 框架（计分/无限复活/50 杀或 8 分钟胜负/复活延迟 3s/HUD）、navmesh 烘焙（M3 用）。M2 前遗留待办：手雷墙体遮挡挡伤（LOS）、近战连击窗口严格化/stab 射程分级、`_in_cone` 垂直差上限。
