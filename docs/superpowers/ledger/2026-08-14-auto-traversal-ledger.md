...
## 实机 r8 验收修复轮（2026-08-16，用户实机 30 分钟 129 attempts 数据驱动）

**实机数据暴露三回归**（GUT 冒烟未覆盖）：①北簇 4 面 **112s**（RimN1 等——箱顶锚点坍缩 0.35-0.4m 恰超 F-degen 阈值 0.3 漏判 → 每重试 32s 退化循环×3）；②东塔区 **26 stuck**（~29s/个——navmesh 替身坡面侧入 quirk 的塔坡道镜像：走廊从西立面中段切入压 0.75m 台阶立面 > step-up 0.62）；③Belt 集群 6 stuck（两处新几何楔点 (2.08,12.07) 门区 / (4.75,-12.23) 摊阁区）。

**修复**：①**F-degen 阈值 0.3→0.6**（物理依据：lerp-8 加速到 relaxed 门 0.6v 需 ~0.5m 助跑）——RimN1 90s→**25.8s success**（GUT 内 RED 复现 5377 帧 → 1547 帧）；**TRAVERSAL_REV r8→r9**（触发语义变更铁律）。②塔坡道侧板实验**否决**（复现 WestTowerBox 回退 stuck——侧板改寻路径形代价>收益，已还原；东区侧入楔死保持 F13 有界兜底，walker 侧/路由侧根治留待）。③记遗留（有界 ~29s）。

**最终基线**：GUT **403/403**。遗留新增：东区侧入楔死（26 面）、Belt 楔点×2、R4 Minor 1、坡脚残余桥面。用户实机 r9 重跑待验收。
