...
- 数据（设计 §3.4）：
  ...
  - 摊位簇×2/街（**T3 裁决间距 ≥1.5m + T4 裁决迁移**：原 z=±7 穿坡道带，迁至 z=5/-11；每簇简化为 1 板+1 箱）：东街簇 N = 板 (18.5,1.1,5) (3,2.2,0.4) + 箱 (15,0.45,5) (1,0.9,1)【板 x∈[17,20] 与箱 x∈[14.5,15.5] 缝恰 1.5，箱贴 rim 面接触】；簇 S = 板 (18.5,1.1,-11) + 箱 (15,0.45,-11)；西街 = 180° 旋转（WestClusterN z=11 / WestClusterS z=-5；旋转对应 EastClusterN↔WestClusterS、EastClusterS↔WestClusterN）
  - 摊阁（翼引力锚）：平台 (18.5,0.6,10) (2.5,1.2,2.5) + 登临台阶 (18.5,0.3,12.0) (1.5,0.6,1.5)【0.6 级，贴摊阁缘无缝】；西街 (-18.5,0.6,-10) + 台阶 (-18.5,0.3,-12.0)（旋转）；摊阁 z∈[8.75,11.25] 与邻板 z∈[6.8,7.2] 缝 1.55 ≥1.5 ✓
...
- 数据（设计 §3.5）：
  ...
  - 市集北带：摊阁×2 (±6,0.6,11.5) (2.5,1.2,2.5)【z∈[10.25,12.75]】+ 台阶 (±7,0.3,13.125) (1.5,0.6,0.75)【贴摊阁北缘 z=12.75 无缝、贴翼墙 z=13.5 无缝、与门柱 x=5 缝 1.25 ≥1.2——2026-08-11 碰撞审计修正：原 (±6,13.25) 1.5 深会侵摊阁/翼墙且与门柱留 0.25 窄缝】；摊簇箱 0.9：北带 MarketN_Box1 (2,0.45,12.5) (1,0.9,1)、MarketN_Box2 (-2,0.45,12.5) (1,0.9,1)；棚板 (±3,4.7,10.75) (4,0.4,2.5)【底 4.5，z∈[9.5,12] 与过梁 z≥12.75 缝 0.75，中央天井 x∈[-1,1]】
...
### 任务 7 [haiku] 外环 + 角场
- 数据（设计 §3.8）：外环视线打断 bigtree×4：(-26.5,1.25,12)/(-26.5,1.25,-12)/(26.5,1.25,-12)/(26.5,1.25,12)（bigtree 格式）；角场×4（以 NW 为例，**2026-08-11 碰撞审计+深度验证终值**：台阶贴缘 23.5；Box2 砍除——原 Box2↔台阶留 0.5m 窄槽，角场 4 实体已足廉价回报+三面可绕；Box1 x 偏移 1.85 真贴缘）：摊阁 (-26,0.6,21.5) (2.5,1.2,2.5) + 台阶 (-26,0.3,23.5) (1.5,0.6,1.5)【贴摊阁缘 z=22.75】+ Box1 (-24.15,0.45,19.65) (1.2,0.9,1.2)【贴摊阁缘 z=20.25 且 x 向投影重叠，真面接触】+ 大树 (-23,1.25,22)（bigtree）。四角：生成器 `_corner_court(cx, cz)`（s 由 cz 符号推导，无 rot 参数）；旋转对 NW↔SE、NE↔SW。OUTER 总数 20（外环树 4 + 角场 4×4）
...
- RED 测试（宏观）：①standable_surfaces().size() == 14 ②每面 top_y ∈ {0.6, 1.2, 2.5, 2.6} ③每面有 name 且全图唯一 ④全实体数 ≤ 220（实体预算警戒线——2026-08-11 裁决：v3 实际 ~189，原 160 为 T1 草拟值作废）⑤≥1.4m 遮挡物（此处用 2.2/3.0 档）≥8 且 7 个探针点 8.89m 内各 ≥1（沿用 v2 test §11.5 探针点集，按 v3 几何微调点位：大厅中心(0,0,0)/市集带(0,±11.5)/市街(±18.5,0)/营前(0,±21.5)/背街(0,±17.75)）⑥全部实体在界内 ⑦旋转对称全表校验（decor/bigtree 豁免装饰差异——v3 无装饰差异，全表严格）
- 验证：v3 + 全套件绿。
